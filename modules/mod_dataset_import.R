#' dataset_import UI Function
#'
#' @description Connects the app to the files for MACH and/or MOPEX.
#' @param id,input,output,session Internal parameters for {shiny}.
#' @import shiny
#' @noRd
#'
mod_dataset_import_ui <- function(id) {
  ns <- NS(id)
  tagList(
    br(),
    fluidRow(
      column(width = 12,
        wellPanel(
          ...
          p("This app allows users to navigate and manipulate the MACH dataset, which contains daily climate and streamflow data along with catchment attributes for 1,014 watersheds within the United States. All tabs retrieve data based on the sites selected on the 'Site Selection' tab.")
        )
      )
    ),
    br(),
    fluidRow(
      column(width = 12,
        wellPanel(
          ...
          p(strong("This application uses data from the MACH dataset, which is consolidated into a single DuckDB file. DuckDB is a relational database management system that offers a client API for R.")),
          tags$ul(
