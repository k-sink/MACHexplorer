wd <- commandArgs(trailingOnly = TRUE)[1] # Shiny app directory
  ...
  if (file.exists(browser_path)) {
    ...
    shiny::runApp(
      ...
      launch.browser = function(shinyurl) {
        system(paste0("\"", browser_path, "\" \"", shinyurl, "\" --incognito"), wait = FALSE)
      }
    )
    ...
    shiny::runApp(appDir = file.path(wd), launch.browser = TRUE)
